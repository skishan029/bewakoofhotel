...
<div class="content">
    <div class="container-fluid">
        <form action="" method="post" id="invoice_form" data-parsley-validate enctype="multipart/form-data">
            <input id="grand_total" type="hidden" name="grand_total" value="{{ $productOrder->grand_total }}">
            <div class="row">
                @forelse ($allproducts as $product)
                    ...
                    @php
                        ...
                        $item = $productOrder->order_items->where('product_id', $product->id)->where('plate_type', '1')->first();
                    ...
                    <div class="col-xl-3 col-lg-3 col-sm-12 col-md-12">
                        <div class="card shadow">
                            <div class="">
                                ...
                                <input id="price_1_{{ $product->id }}" class="form-control rounded-0" type="hidden" name="prices[]" value="{{ blank($item) ? $product->full_price : $item->price }}">
                                <input id="total_1_{{ $product->id }}" class="form-control rounded-0 total" type="hidden" name="totals[]" value="{{ blank($item) ? '' : $item->total }}">
                                <input id="product_name_1_{{ $product->id }}" class="form-control rounded-0" type="hidden" value="{{ $product->product_name }} - Full">
                                ...
                                {{-- <div class="row">
                                    ...
                                    <div class="col-6">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" readonly value="0" id="quantity_1_{{ $product->id }}" name="quantitys[]" value="{{ blank($item) ? '0' : intval($item->quantity) }}">
                                        ...
                                        <h6><b>&#8377;{{ blank($item) ? number_format($product->full_price, '2', '.', '') : number_format($item->price, '2', '.', '') }}</b></h6>
                                    </div>
                                </div> --}}
                                ...
                                <div class="row">
                                    ...
                                    <!--<div class="col-7">
                                        <h6 style="height:55px;">
                                            @if (!empty($product->featured_photo))
                                                <img class="img-fluid" src="{{ Storage::url($product->featured_photo) }}" alt="{{ $product->product_name }}" style="width: 40px; height:23px;">
                                            @endif
                                    ...
                                    <div class="col-6">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" readonly id="quantity_1_{{ $product->id }}" name="quantitys[]" value="{{ blank($item) ? '0' : intval($item->quantity) }}">
                                            <div class="input-group-append">
                                                <button type="button" id="lessqty_{{ $product->id }}" class="btn btn-danger btn-sm" value="{{ $jsonArr }}" onclick="lessQty(this)"><i class="fas fa-minus"></i></button>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-12">
                                        <strong>&#8377;{{ blank($item) ? number_format($product->full_price, '2', '.', '') : number_format($item->price, '2', '.', '') }}</strong>
                                    </div>-->
                                </div>
                            </div>
                        </div>
                    ...
                    @if (!empty($product->half_price))
                        ...
                        @php
                            ...
                            $item = $productOrder->order_items->where('product_id', $product->id)->where('plate_type', '2')->first();
                        @endphp
                        <div class="col-xl-3 col-lg-3 col-sm-12 col-md-12">
                            <div class="card shadow">
                                <div class="">
                                    ...
                                    <input id="price_2_{{ $product->id }}" class="form-control rounded-0" type="hidden" name="prices[]" value="{{ blank($item) ? $product->half_price : $item->price }}">
                                    <input id="total_2_{{ $product->id }}" class="form-control rounded-0 total" type="hidden" name="totals[]" value="{{ blank($item) ? '' : $item->total }}">
                                    ...
                                    <div class="row">
                                        ...
                                        {{-- <div class="col-5">
                                            <div class="input-group input-group-sm">
                                                ...
                                                <input type="text" class="form-control" readonly id="quantity_2_{{ $product->id }}" name="quantitys[]" value="{{ blank($item) ? '0' : intval($item->quantity) }}">
                                                <div class="input-group-append">
                                                    <button type="button" id="lessqty_{{ $product->id }}" class="btn btn-danger btn-sm" value="{{ $jsonArr }}" onclick="lessQty(this)"><i class="fas fa-minus"></i></button>
                                                </div>
                                            </div>
                                        ...
                                        <div class="col-12">
                                            <strong>&#8377;{{ blank($item) ? number_format($product->half_price,'2','.', '') : number_format($item->price,'2','.', '') }}</strong>
                                        </div> --}}
                                        ...
                                        <div class="col-6">
                                            ...
                                            <h6><b>&#8377;{{ blank($item) ? number_format($product->half_price,'2','.', '') : number_format($item->price,'2','.', '') }}</b></h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input id="date_time" class="form-control rounded-0" type="text" readonly disabled value="{{ date('d-m-Y g:i a', strtotime($productOrder->created_at)) }}">
                                    </div>
                                </div>

                                @php
                                    $payment_option = $productOrder->payment_option;
                                @endphp
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select id="payment_option" class="form-control rounded-0" name="payment_option" onchange="setTransactionNumber(this.value)">
                                            @foreach (Helper::getPaymentOption() as $key=> $item)
                                                <option value="{{ $key }}" @selected($payment_option == $key) >{{ $item }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input id="name" class="form-control rounded-0" type="text" name="name" value="{{ $productOrder->name }}">
                                    </div>
                                </div>

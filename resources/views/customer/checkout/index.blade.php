                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6">
                                    <div class="form-inner">
                                        ...
                                        <select name="region_id" id="region_id"
                                            onchange="getSubRegions('region_id','subregion_id')">
                                        ...
                                        <div class="nice-select" tabindex="0"><span
                                                class="current">{{ optional($parentRegions->firstWhere('id', $region_id))->name ?? 'Select Region' }}</span>
                                            <ul class="list">
                                                @foreach ($parentRegions as $item)
                                                    <li data-value="{{ $item->id }}"
                                                        class="option @selected($region_id == $item->id)">{{ $item->name }}
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-inner">
                                        ...
                                        <div class="nice-select" tabindex="0"><span
                                                class="current">{{ optional($subRegions->firstWhere('id', $subregion_id))->name ?? 'Select Sub Region *' }}</span>
                                            <ul class="list">
                                                @if ($subRegions->isNotEmpty())
                                                    @foreach ($subRegions as $item)
                                                        <li data-value="{{ $item->id }}"
                                                            class="option @selected($subregion_id == $item->id)">{{ $item->name }}
                                                        </li>
                                                    @endforeach
                                                @endif
                                            </ul>
                                    ...
                                    <li class="single-product d-flex justify-content-start">
                                        <div class="product-info">
                                            ...
                                            <div class="product-total d-flex align-items-center">
                                                <strong><span class="product-quantity">{{ $item->qty }}</span> <i
                                                        class="bi bi-x-lg px-2"></i>
                            ...
                            <table class="table cost-summery-table">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        @php
                                            $shippingCharge = Helper::getShippingCharge();
                                        @endphp
                                        @if ($shippingCharge == 0)
                                            <td>Free</td>
                                        @else
                        ...
                        <div class="summery-card total-cost mb-10">
                            <table class="table cost-summery-table total-cost">
                                <thead>
                                    <tr>
                                        <th>Total</th>
                                        <th>₹ {{ number_format(floatval($subTotal) + $shippingCharge, 2) }}</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="payment-methods mb-10">
                            <div class="form-check payment-check">
                                <input class="form-check-input" type="radio" name="payment_option"
                                    id="flexRadioDefault2" value="cash" checked>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#checkout-form').on('submit', function(e) {
                    ...
                    var subregion_id = $('#subregion_id').next('.nice-select').find('.option.selected').data(
                        'value');

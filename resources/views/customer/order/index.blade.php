        ...
        <div class="row g-4">
                    ...
                    <table class="table total-table">
                        <thead>
                            <tr>
                                ...
                                @endphp
                                <th><span style="background: {{ $OrderStatusArr['bg'] }}; padding: 4px 10px; border-radius: 4px;">
                                ...
                                <td>
                                    <ul class="cost-list text-start">
                                        @foreach ($order->order_items as $item)
                                            @php
                                                $product_details = empty($item->product_details) ? [] : json_decode($item->product_details, true);
                                            @endphp
                                            <li>
            ...
            @empty
                <div class="card" style="text-align: center; padding: 4rem;">
                    ...
                    <a href="{{ route('customer.product.index') }}" class="action-btn" style="margin-top: 1rem;">Order Something</a>
                </div>
            @endforelse
        </div>
